...
describe('list reducer', () => {
  ...
  describe('removeList', () => {
    ...
    it('selects the default list if removed list was selected', () => {
      ...
      expect(store.getState()).toEqual({
        byUrl: {},
        selectedListUrl: 'https://raw.githubusercontent.com/valuenetworklive2021/valueswap-files/master/tokenlists/valueswap-default.tokenlist.json'
      })
    })
  })
  ...
  describe('updateVersion', () => {
    describe('never initialized', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          byUrl: {
            'https://unpkg.com/@valueswap/default-token-list@latest/valueswap-default.tokenlist.json': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
            },
            'https://unpkg.com/@valueswap/default-token-list@latest': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
      ...
      it('clears the current lists', () => {
        expect(
          store.getState().byUrl['https://unpkg.com/@valueswap/default-token-list@latest/valueswap-default.tokenlist.json']
        ).toBeUndefined()
        expect(store.getState().byUrl['https://unpkg.com/@valueswap/default-token-list@latest']).toBeUndefined()
      })
    ...
    describe('initialized with a different set of lists', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          ...
          selectedListUrl: undefined,
          lastInitializedDefaultListOfLists: ['https://unpkg.com/@valueswap/default-token-list@latest']
        })
        store.dispatch(updateVersion())
      })
      ...
      it('each of those initialized lists is empty', () => {
        ...
        // note we don't expect the valueswap default list to be prepopulated
        // this is ok.
        Object.keys(byUrl).forEach(url => {
          if (url !== 'https://unpkg.com/@valueswap/default-token-list@latest/valueswap-default.tokenlist.json') {
