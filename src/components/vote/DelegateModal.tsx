...
export default function DelegateModal({ isOpen, onDismiss, title }: VoteModalProps) {
  ...
  const vntwBalance = useTokenBalance(account ?? undefined, chainId ? VNTW[chainId] : undefined)
      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          <AutoColumn gap="lg" justify="center">
            ...
            <TYPE.body>Earned VNTW tokens represent voting shares in Valueswap governance.</TYPE.body>
            <TYPE.body>
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOndismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.main fontSize={36}>{vntwBalance?.toSignificant(4)}</TYPE.main>
          </AutoColumn>
        </LoadingView>
      )}
      {hash && (
