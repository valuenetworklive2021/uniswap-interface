...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WETH } from '@valueswap/sdk'
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getEtherscanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">Valueswap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into Valueswap V2 at a price you believe is correct. If the V2 price seems
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on Valueswap V2. Your liquidity will be migrated at the
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your Valueswap V1 ${token.symbol}/ETH liquidity will become Valueswap V2 ${token.symbol}/ETH liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `VNTW-V1-${token.symbol}`, 'Valueswap V1')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <div>
            <QuestionHelper text="Migrate your liquidity tokens from Valueswap V1 to Valueswap V2." />
          </div>
        </AutoRow>
        ...
        {!account ? (
          ...
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because Valueswap V2 uses WETH under the hood, your Valueswap V1 WETH/ETH liquidity cannot be migrated. You
